...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
     ...
     */
    def sanitized: Observable[String] = Observable.create[String](observer => {
      obs.map(s => s.replace(" ","_")) .subscribe(
        term => {
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
     ...
     */
    def recovered: Observable[Try[T]] = Observable.create[Try[T]](observer => {
      obs.map(x => Success(x)).onErrorReturn(e => Failure(e)).doOnCompleted(observer.onCompleted()) subscribe(term =>{observer.onNext(term)})
      Subscription()
    })
     ...
     */
    def timedOut(totalSec: Long): Observable[T] = Observable.create[T](observer => {

        obs.take(totalSec second)subscribe(
        term => {
          observer.onNext(term)
        },
        t => {
     ...
     */
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = Observable.create[Try[S]](observer => {
      obs.concatMap(t => requestMethod(t).recovered) subscribe(
        term => {
          observer.onNext(term)
        },
        e => {
