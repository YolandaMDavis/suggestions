...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val searchTerms: Observable[String] =  Observable.create[String](observer => {
      searchTermField.textValues subscribe (x => {
       observer.onNext(x)
      },e=>{println(e)})
      Subscription()
    })
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] =  Observable.create[Try[List[String]]](observer => {
      searchTerms subscribe (t=>{
        wikiSuggestResponseStream(t).recovered subscribe (x => {observer.onNext(x)})
      },e=>{println(e)})
      Subscription()
    })
    ...
    val suggestionSubscription: Subscription =  suggestions.observeOn(eventScheduler) subscribe {
      x:Try[List[String]] => x match{
          ...
          case Failure(e) => {
            status.text = "Error Loading Suggestion: " + e
          }
      }
    }
    ...
    val selections: Observable[String] = Observable.create[String](observer => {
      button.clicks subscribe( x=>{
        if(x.text.eq("Get") && suggestionList.selection.items.nonEmpty) {
          observer.onNext(suggestionList.selection.items.head)
        }
      })
      Subscription()
    })
    ...
    // TO IMPLEMENT
    val pages: Observable[Try[String]] =  Observable.create[Try[String]](observer => {
      selections subscribe(x => {
        wikiPageResponseStream(x).recovered subscribe (a => {
          observer.onNext(a)
        })
      })
      Subscription()
    ...
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      ...
      x => x match  {
        ...
        case Failure(e) => {
          status.text = "Error loading page" + e
        }
      }
    }
